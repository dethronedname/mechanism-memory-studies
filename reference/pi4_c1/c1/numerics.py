...
from pi4.learning import model_for,objective,METHODS
...
def preflight(c,datadir,device,out,allow_cpu_test=False):
    ...
    result={'status':'PASS' if all(x['pass'] for x in rows) else 'FAIL','checks':rows,
        'actual_device':device,'scope':'15 finite FP32 training-path triplets with clipped AdamW; CPU64 reference is not a GPU performance result.'}
    write(out,result)
    ...
    for method in config['methods']:
      for seed in config['seeds']:
        ...
        try:
            ...
            curve=read(td/'curve.json');result=read(td/'result.json');best=min(curve,key=lambda r:r['selection_score'])
            ...
            m=Model(**ck['config']);m.load_state_dict(ck['state_dict']);m.eval()
            from pi4.experiment import valid
