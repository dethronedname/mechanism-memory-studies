...
import numpy as np
import torch
from pi4.io import write,sha,seed_all,load,take
from pi4.learning import objective,general_views,reset_views
from .models import METHODS,model_for
...
def fit(config,method,seed,datadir,out,device,started=None):
    ...
    result={'status':'COMPLETE' if done==config['updates'] else 'BUDGET_STOP','method':method,'family':METHODS[method][0],'seed':seed,'updates':done,
            ...
            'learning_rate':config['learning_rate'],'peak_cuda_allocated_bytes':int(torch.cuda.max_memory_allocated()) if str(device).startswith('cuda') else None,
